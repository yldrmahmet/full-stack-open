...
// body every time has 2 parameters
...
test("a note can be updated", async () => {
  ...
  const newNote = {
    content: `${noteToUpdate.content} and updated`,
  ...
  assert.strictEqual(updatedNote.content, newNote.content);
  assert.strictEqual(updatedNote.important, !noteToUpdate.important);
